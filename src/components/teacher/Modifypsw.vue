...
export default {
  ...
  methods: {
    async editPassword () {
      ...
      this.teacherId = window.sessionStorage.getItem('teacher_id')
      const { data: res } = await this.$http.post(`teacher?action=modifyPassword&teacherId=${this.teacherId}&oldPassword=${this.ruleForm.beforePass}&newPassword=${this.ruleForm.checkPass}`)
